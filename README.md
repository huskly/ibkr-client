...
chunk starts only after the current chunk succeeds. A terminal failure cancels queued definitions
that belong to that operation. It does not cancel definitions that belong to another operation.
HTTP requests that started before cancellation can settle normally.
